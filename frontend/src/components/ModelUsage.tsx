...
export function ModelUsage() {
  ...
  return (
    <Card className="flex flex-col">
      ...
      {models.length === 0 ? (
        <p className="mt-2 text-sm text-gray-400">No model calls yet</p>
      ) : (
        <div className="mt-2 space-y-2">
          {models.map((m) => (
            <div key={m.model} className="rounded-lg bg-gray-50 px-3 py-2">
              <div className="font-mono text-xs text-gray-800">{m.model.split('/').pop()}</div>
